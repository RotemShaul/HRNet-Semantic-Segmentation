...
class CityscapesRGBDC2c(BaseDataset):
    def __init__(self, 
                 root, 
                 list_path, 
                 num_samples=None, 
        ...
        super(CityscapesRGBDC2c, self).__init__(ignore_label, base_size,
    ...
    def read_files(self):
        ...
        if 'test' in self.list_path:
            ...
            for item in self.img_list:
                ...
                confidence_path = image_path.replace('leftImg8bit/', 'confidence/', 1)
    ...
    def __getitem__(self, index):
        ...
        confidence_map = Image.read(confidence_full_path) ###cv2.imread(os.path.join(self.root,'cityscapes',item["confidence"]), ###cv2.IMREAD_GRAYSCALE)
        ...
        _, disparity = self.gen_sample(image_copy, disparity,
                                self.multi_scale, self.flip,
                                self.center_crop_test, disparity_transform=True, mean_disp=self.mean_disp, std_disp=self.std_disp,  normalize_disparity =self.normalize_disparity)
        ...
        return image.copy(), label.copy(), np.array(size), name, disparity.copy(), confidence_map.copy()
